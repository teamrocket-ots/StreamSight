...
def _show_components(df_mqtt):
    ...
    st.plotly_chart(mqtt_delay_components(df_mqtt), **STRETCH, key="mqtt_analysis_chart_1")
    ...
    if "bottleneck" in df_mqtt.columns and df_mqtt["bottleneck"].notna().any():
        ...
        st.plotly_chart(fig, **STRETCH, key="mqtt_analysis_chart_2")
...
def _show_delay(df_mqtt, column, label, colour):
    ...
    if "timestamp" in data.columns:
        ...
        st.plotly_chart(fig, **STRETCH, key=f"mqtt_time_{column}")
    ...
    if category_col in df_mqtt.columns and df_mqtt[category_col].notna().any():
        ...
        st.plotly_chart(fig, **STRETCH, key=f"mqtt_cat_{column}")
...
def _draw_topology(df_mqtt, brokers, clients):
    ...
    st.plotly_chart(fig, **STRETCH, key="mqtt_analysis_chart_5")
